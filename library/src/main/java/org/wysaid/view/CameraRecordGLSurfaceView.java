...
public class CameraRecordGLSurfaceView extends CameraGLSurfaceViewWithTexture {
    ...
    public void startRecording(final String filename, final StartRecordingCallback recordingCallback) {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                if (!mOriginFrameRecorder.startRecording(30, filename) && !mFrameRecorder.startRecording(30, filename)) {
    ...
    public void endRecording(final EndRecordingCallback callback, final boolean shouldSave) {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                if (mFrameRecorder != null) {
                    mFrameRecorder.endRecording(false);
                    mOriginFrameRecorder.endRecording(shouldSave);
                }
                if (callback != null) {
...
//                audioData = new short[bufferSize];
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (mShouldRecord && bufferReadResult > 0 && mOriginFrameRecorder != null &&
                        mOriginFrameRecorder.getTimestamp() > mOriginFrameRecorder.getAudioStreamtime()) {
//                    Log.e(LOG_TAG, "buffer Result: " + bufferReadResult);
                    audioBuffer.position(0);
//                    audioBuffer.put(audioData).position(0);
                    mOriginFrameRecorder.recordAudioFrame(audioBuffer, bufferReadResult / 2);
                }
            }
